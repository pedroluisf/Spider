...
(function (vis) {
    ...
    function reCalc(d) {
        ...
        lCom.showMessage(d.supplier.shortName + ' to ' + d.borrower.shortName);
    ...
    function initNodes(data) {
        ...
        if (data) {
            ...
            while (--i > -1) {
                ...
                n = getChild(d);
                d.nodes.push(n);
                n.ext.currents[d.seq] = (n.ext.currents[d.seq] || 0);
                n.ext.currents[d.seq]++;
    ...
    function lme(d) {
        selectedExt = d.value;
        var title = cat == 'loadTimeMS' ? 'Load Time' : 'Mime Type';
        var value = d.key + (cat == 'loadTimeMS' ? ' ms' : '');
        tooltip.html([
            "<b style='text-shadow: 1px 1px 1px #000; color: #FFFFFF'>", title, "</b>",
            "<hr>",
    ...
    function showToolTip(d) {
        ...
        if (tooltip.style("display") == "none") {
            ...
            var headerStyle = 'style="text-shadow: 0 0 2px rgba(0, 0, 0, .8); color: #FFFFFF"';
            if (d.type == typeNode.parent) {
                res = [
                    '<strong ' + headerStyle + '>Country: </strong>' + d.nodeValue.name
                ];
            }
            else {
                res = [
                    '<strong ' + headerStyle + '>Mime: </strong>' + '<strong style="color: ' + extColor(d.nodeValue["fileType"]) + '">' + d.nodeValue.fileType + '</strong>',
                    '<strong ' + headerStyle + '>Load Time: </strong>' + d.nodeValue.loadTimeMS.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' ms',
                    '<hr/><strong ' + headerStyle + '>IP: </strong>'+ d.nodeValue.ip,
                    '<strong ' + headerStyle + '>ISP: </strong>' + d.nodeValue.isp,
                    '<strong ' + headerStyle + '>URL: </strong>' + d.nodeValue.url.substring(0, 30) + (d.nodeValue.url.length > 30 ? '...' : ''),
                    '<hr/><strong ' + headerStyle + '>Country: </strong>' + d.nodeValue.destination.country,
                    '<strong ' + headerStyle + '>State: </strong>' + d.nodeValue.destination.state,
                    '<strong ' + headerStyle + '>City: </strong>' + d.nodeValue.destination.city
                ];
            }

            tooltip.html(res.join("</br>"));
            tooltip.style("display", "block");
        }
    }
...
(function () {
    ...
    function initLocal(obj) {
        var xy = coord(obj.lng, obj.lat);

        var local = {
              key: obj.country,
              name: obj.country,
              shortName: obj.country ? obj.country.split(',')[0] : '',
              x: xy.x,
              y: xy.y,
    ...
    function initItem(d) {
        ...
        d.value = d.basevalue = d["loadTimeMS"];
        d.valueOf = value;
        d.borrower = initLocal(d.destination);
        d.borrower.borrowed = 0;
        d.borrower.loadTimeMS = d.loadTimeMS;
        d.supplier = initLocal(d.source);
        d.supplier.supplied = 0;
        d.supplier.loadTimeMS = d.loadTimeMS;
        d.Region = "XXX";
    ...
    function request(error, data) {
        ...
        while (--l > -1) {
            ...
            a.parent = a.supplier;
            b.parent = b.borrower;
            a.seq = a.seq - stepDate /*/2*/;
            _data.push(a);
            _data.push(b);
        }
        _data.sort(sortBySeq);
        ...
        setting.showEdge = setting.originalShowEdge;
    ...
    d3.select("#chEdge").on("change", function (d) {
        ...
        setting.originalShowEdge = this.checked;
    });
    ...
    App.loadData = function (valueSelected) {
        d3.json("world-countries.json", function (error, collection) {
            ...
            d3.json('http://localhost:3000/pageobjects/' + valueSelected, request);
            // For example data
			//request(null, App.pageObjects[valueSelected]);
        });
    };

    App.Dropdown.load();
    App.Dropdown.attachEventHandlers();
})();
